...
export const attributeClient = {
  ...crudFactory<Attribute, QueryOptions, CreateAttributeInput>(API_ENDPOINTS.ATTRIBUTES),
  paginated: ({ type, name, shop_id, ...params }: Partial<AttributeQueryOptions>) => {
    return HttpClient.get<AttributePaginator>(API_ENDPOINTS.ATTRIBUTES, {
      searchJoin: 'and',
      ...params,
      shop_id: shop_id, // Directly include shop_id
      type,
      name,
    });
  },
