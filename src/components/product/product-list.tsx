...
const ProductList = ({
  ...
  const handleShowQuantity = (id: string) => {
    setEditMode((prev) => ({
      ...prev,
      [id]: true,
    }));
  ...
  let columns = [
    ...
    },
    {
      ...
      render: (quantity: number, record: Product) => {
        const isEditing = editMode[record.id];
          ...
          <div>
            {isEditing ? (
              <>
                ...
                />
                <Button
                  onClick={() => handleEditQuantity(record.id, editedQuantity)}
                  size="small"
                  className="mt-2"
                >
                  Update
              ...
              <span
                onClick={() => handleShowQuantity(record.id)}
                className="font-semibold text-accent underline transition-colors duration-200 ms-1 hover:text-accent-hover hover:no-underline focus:text-accent-700 focus:no-underline focus:outline-none"
              >
                {quantity}
              </span>
