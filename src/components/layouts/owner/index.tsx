...
const OwnerLayout: React.FC<{ children?: React.ReactNode }> = ({
  ...
  useEffect(() => {
    if (data && data.shops && data.shops.length > 0) {
      const newShopSlug = data.shops.map((shop)=> {shop.slug});
      console.log("newShopssssss", newShopSlug)
  ...
  if (router.pathname === Routes.OwnerMyShops) {
    matchedLinks = [
      {
...
// }
    ...
    matchedLinks = permissions?.includes('owner')
      ? siteSettings.sidebarLinks.owner
      : siteSettings.sidebarLinks.owner.filter((link) =>
