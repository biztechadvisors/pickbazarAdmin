...
const SidebarItem = ({ href, icon, label, description, helpLink }: any) => {
  const { closeSidebar } = useUI();
  ...
  if (label === 'Inventory') {
    return (
      ...
      <InventoryDropdown description={description} />
      </div>
    );
  }

  // Render the regular sidebar item if the label is not "Orders" or "Inventory"
  return (
    <Link
      href={href}
      className="flex w-full items-center text-base text-body-dark text-start focus:text-accent"
