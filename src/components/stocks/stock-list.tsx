...
const StockList = ({ me, onSort, onOrder }: IProps) => {
  ...
  const { data: stocks } = useGetStock(me?.id);
  ...
  let columns = [
    ...
    },
    {
      title: (
        <TitleWithSort
          title={t('table:table-item-title')}
          ascending={sortingObj.sort === 'asc' && sortingObj.column === 'name'}
          isActive={sortingObj.column === 'name'}
        />
      ),
      className: 'cursor-pointer',
    ...
    },
    {
      ...
      key: 'ordPendQuant',
      align: 'center' as const,
      render: (ordPendQuant: number, record: any) => (
        <EditablePendingQuantity record={record} me={me} />
      ),
    },
    {
      title: t('Quantity'),
      ...
      align: 'center' as const,
      render: (quantity: number, record: any) => (
        <EditableQuantity record={record} me={me} />
      ),
    },
  ];
