...
export default function Uploader({
  ...
  const BASE_URL =
    process.env.NEXT_PUBLIC_API_URL || 'https://admin-bakend.codenoxtech.com';
    ...
    onDrop: async (acceptedFiles) => {
      if (acceptedFiles.length) {
        upload(acceptedFiles, {
          onSuccess: (data) => {
            // 🧠 Fix: Ensure correct full URL for deployed environments
            data = data.map((file, idx) => {
              ...
              // ✅ Ensure full image path (absolute URL)
              const thumbnailPath = file?.thumbnail?.startsWith('http')
                ? file.thumbnail
                : `${BASE_URL}${file.thumbnail.startsWith('/') ? '' : '/'}${file.thumbnail}`;
...
//       {console.log('thumbs.length, loading', thumbs.length, thumbs);}
