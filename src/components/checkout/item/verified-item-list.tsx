...
const VerifiedItemList: React.FC<Props> = ({ className }) => {
  ...
  const { price: tax } = usePrice({ amount: 0 }); // Adjust tax logic if needed
  const { price: shipping } = usePrice({ amount: 0 }); // Adjust shipping logic if needed
  ...
  switch (coupon?.type) {
    case CouponType.PERCENTAGE:
      ...
      break;
    case CouponType.FREE_SHIPPING:
      calculateDiscount = 0; // Free shipping logic (adjust if needed)
      break;
    default:
      calculateDiscount = Number(discount);
  }

  const { price: discountPrice } = usePrice({ amount: calculateDiscount });
  ...
  const totalPrice = calculatePaidTotal(
    {
      totalAmount: base_amount,
      tax: 0, // Adjust tax if needed
      shipping_charge: 0, // Adjust shipping if needed
    },
    Number(calculateDiscount)
  );
